...
Set-NetFirewallProfile -all 2>nul
SET MYERROR=%ERRORLEVEL%
echo Set-NetFirewallProfile -all Error=%MYERROR%
echo Set-NetFirewallProfile -all Error=%MYERROR%  >> log.txt

netsh advfirewall firewall add rule name=«telemetry_vortex.data.microsoft.com» dir=out action=block remoteip=191.232.139.254 enable=yes 2>nul
SET MYERROR=%ERRORLEVEL%

netsh advfirewall firewall add rule name=«telemetry_telecommand.telemetry.microsoft.com» dir=out action=block remoteip=65.55.252.92 enable=yes 2>nul
SET MYERROR=%ERRORLEVEL%

netsh advfirewall firewall add rule name=«telemetry_sqm.telemetry.microsoft.com» dir=out action=block remoteip=65.55.252.93 enable=yes 2>nul
SET MYERROR=%ERRORLEVEL%

netsh advfirewall firewall add rule name=«telemetry_watson.telemetry.microsoft.com» dir=out action=block remoteip=65.55.252.43,65.52.108.29 enable=yes 2>nul
SET MYERROR=%ERRORLEVEL%

netsh advfirewall firewall add rule name=«telemetry_redir.metaservices.microsoft.com» dir=out action=block remoteip=194.44.4.200,194.44.4.208 enable=yes 2>nul
SET MYERROR=%ERRORLEVEL%
